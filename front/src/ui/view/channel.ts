...
class ChannelView implements UiComponent {
	...
	render(data: ChannelInfo) {
		...
		this.avatar.src = data.avatar
			? thumbnailMiddle + data.avatar
			: "/favicon.svg";
		this.banner.src = data.banner
			? thumbnailMiddle + data.banner
			: "/favicon.svg";
	...
	mount(): void {
		...
		this.channelSubscribeBtn.addEventListener("click", async () => {
			...
			const q = res
				? "user -subs! s._id = " + id
				: "user updateOneOrAdd subs s._id = " +
					id +
					" u.last=" +
		...
		this.channelShareBtn.addEventListener("click", () => {
			navigator.clipboard.writeText(
				"https://youtube.com/channel/" + $store.channelId.get(),
			);
	...
	async load(id: string) {
		...
		const data = await fetchVQL<ChannelInfo>(
			"api channelInfo! s._id = " + id + " s.update = true",
		);
		this.render(data);
		setTimeout(() => this.loadVideos(id), 100);
		this.loadVideosButton.addEventListener("click", () =>
			this.loadVideos(id, false),
		);
		this.show();
	}
