...
export function toggleFullscreen() {
	playerView.element.classList.contains("fullscreen")
		...
		: playerView.element.requestFullscreen();
...
export async function loadProgress() {
	...
	if (time) {
		...
		if (
			time + 3 >
			(playerView.mediaSync.getDuration() || $store.video.get().duration || 0)
		)
			time = 0;
...
async function loadVideoFn(id: string, opts: Partial<LoadVideoOpts> = {}) {
	...
	fetchVQL(
		`user updateOneOrAdd history s._id=${id} u.watched=true u.last=$_nowShort`,
	).then(() => {
