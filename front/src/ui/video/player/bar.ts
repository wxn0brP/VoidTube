...
export function setupBar() {
	...
	const fullscreenBtn = playerView.bar.qs<HTMLButtonElement>(
		"toggle-fullscreen",
		1,
	);
	...
	document.addEventListener("mousemove", () => {
		...
		full_timeout = setTimeout(() => {
			...
			if (playerView.element.classList.contains("fullscreen"))
				setTimeout(() => {
					playerView.videoEl.style.cursor = "none";
				}, 1000);
		}, 2000);
	});
	...
	playerView.mediaSync.eventEmitter.on("timeupdate", () => {
		...
		if (
			!$store.player.loop.get() &&
			round(playerView.mediaSync.currentTime, 1) + 0.1 >=
				round(playerView.mediaSync.getDuration(), 1)
		) {
			playNextDebounced();
		}
		...
		if (
			playerView.mediaSync.currentTime + 13 >=
				playerView.mediaSync.getDuration() &&
			!$store.player.loop.get()
		) {
			bufferNextThrottled();
		}
	});
	playerView.mediaSync.eventEmitter.on(
		"ended",
		() => !$store.player.loop.get() && playNextDebounced(),
	);
	playerView.mediaSync.eventEmitter.on("ended", () =>
...
export function updateProgressBars() {
	...
	if (Number.isNaN(duration) || duration === Infinity) return;
...
document.addEventListener("keydown", e => {
	...
	if (e.code === "ArrowRight") {
		const time = Math.min(
			playerView.mediaSync.getDuration(),
			playerView.mediaSync.currentTime + 5,
		);
	...
	if (!Number.isNaN(parseInt(e.key))) {
		...
		const time = clamp(
			0,
			playerView.mediaSync.getDuration() * (value / 100),
			playerView.mediaSync.getDuration(),
		);
		playerView.mediaSync.seek(time);
	}
