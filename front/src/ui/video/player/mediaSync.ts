...
class MediaSyncController {
	...
	private events() {
		...
		audio.addEventListener("loadeddata", (...args) =>
			this.eventEmitter.emit("loadeddata", ...args),
		);
		audio.addEventListener("timeupdate", (...args) =>
			this.eventEmitter.emit("timeupdate", ...args),
		);
		audio.addEventListener("seeking", (...args) =>
			this.eventEmitter.emit("seeking", ...args),
		);
		audio.addEventListener("ended", (...args) =>
			this.eventEmitter.emit("ended", ...args),
		);
		audio.addEventListener("pause", (...args) => {
			if (loadedTime + 1000 > Date.now())
				return this.eventEmitter.emit("play", ...args);
			this.eventEmitter.emit("pause", ...args);
		});
		audio.addEventListener("play", (...args) =>
			this.eventEmitter.emit("play", ...args),
		);
		audio.addEventListener("error", (...args) =>
			this.eventEmitter.emit("error", ...args),
		);
		audio.addEventListener("progress", (...args) =>
			this.eventEmitter.emit("progress", ...args),
		);
	}
	...
	private syncClients() {
		...
		if (this.videoEnabled && this.isUnSync(this.video))
			this.video.currentTime = this.clockTime;
	}
