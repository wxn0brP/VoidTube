...
class MetaControlView implements UiComponent {
	...
	public async toggleToPlayList(id = $store.videoId.get(), e?: Event) {
		if (e && e instanceof MouseEvent && e.shiftKey) {
			...
			fetchVQL(
				`user updateOneOrAdd playlist s._id=later u.last=$_nowShort add_arg.name = "Watch later"`,
			);
		...
		playListsModal.show({
			callback: async playlistId => {
				...
				await fetchVQL(
					`playlist ${op} ${playlistId} ${playlist.has ? "s" : "d"}._id = ${id}`,
				);
			...
			reRenderCallback: () => {
				playListsModal.elements.forEach(item => {
					const has =
						playlists.find(playlist => playlist._id === item.id)?.has ?? false;
	...
	public async download() {
		...
		formats.forEach(f => {
            ...
            `;
			if (f.is_audio && !f.is_video)
				onlyAudio.push([
					f.url,
					option,
				]);
			else if (f.is_video && !f.is_audio)
				onlyVideo.push([
					f.url,
					option,
				]);
