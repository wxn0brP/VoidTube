...
export async function clear(cmp: QueuePanel, confirm = false, silent = false) {
	if (
		confirm &&
		!(await uiFunc.confirm("Are you sure you want to clear the queue?"))
	)
		return;
	cmp.queue = [];
	cmp.videoMap.clear();
