		...
		if (fromHistory.includes(s)) {
			...
			remove.addEventListener("click", async e => {
				...
				const confirm = await uiFunc.confirm(
					"Are you sure you want to remove this search from history?",
				);
				if (!confirm) return;

				cmp.searchHistory = cmp.searchHistory.filter(h => h !== s);
				fetchVQL(`user -search-history s._id = "${s}"`, {
					silent: true,
				});
				renderSuggestions(
					cmp,
					suggestions,
					fromHistory.filter(h => h !== s),
				);
			});
		}
...
export async function handleAutocomplete(cmp: SearchBarView, query: string) {
	...
	try {
		...
		const apiSuggestions = await fetchVQL<string[]>(q, {
			silent: true,
		});
