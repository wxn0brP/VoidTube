...
export async function fetchQuickCache(id: string) {
	...
	const staticData = await db.cache["video-static"].findOne({
		_id: id,
	});
	if (staticData) {
		staticData.channelName = await channelInfo(staticData.channel).then(
			c => c.name,
		);
		return staticData;
	}
...
export async function fetchQuickCache$(search: {
	...
	// @ts-ignore
	const staticData = await db.cache["video-static"].find({
		$in: {
			_id: missingIds,
		},
	});
