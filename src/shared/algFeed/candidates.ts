...
export async function buildInitialCandidates(
	history: Video[],
	config: Config,
): Promise<SearchEntry[]> {
	if (history.length < config.minHistory) {
		note(
			"alg-buildInitialCandidates",
			"Not enough history. Returning empty array.",
		);
		return [];
	}
	...
	for (const keyword of keywords) {
		...
		note(
			"alg-buildInitialCandidates",
			`Found ${results.length} videos for "${keyword}"`,
		);
	...
	note(
		"alg-buildInitialCandidates",
		`Found a total of ${allResults.length} videos`,
	);
	...
	note(
		"alg-buildInitialCandidates",
		`After deduping, found ${unique.size} videos`,
	);
...
export function getTopKeywordsFromHistory(
	history: Video[],
	config: Config,
): string[] {
