...
const manifestUrl =
	"https://raw.githubusercontent.com/wxn0brP/VoidTube/refs/heads/dist-split/output/manifest.json";
...
async function downloadAndAssemble(manifestUrl: string, outputDir: string) {
	...
	console.log(
		logPrefix,
		`Output file: ${outputFile} ${fs.existsSync(outputFile)}`,
	);
	console.log(
		logPrefix,
		`App commit version file: ${gitvvPath} ${fs.existsSync(gitvvPath)}`,
	);
	...
	if (fs.existsSync(gitvvPath)) {
		...
		if (currentVersion === manifest.commit) {
			console.log(
				logPrefix,
				`Current version (${currentVersion}) matches manifest. Skipping update.`,
			);
			...
			console.log(
				logPrefix,
				`Local version (${currentVersion}) != remote (${manifest.commit}), updating...`,
			);
		}
	}
	...
	console.log(
		logPrefix,
		`Successfully downloaded and assembled ${outputFile} with valid checksum.`,
	);
...
const resourcesDir = path.dirname(process.argv[0]).includes("electron")
	? path.join(import.meta.dirname, "..", "..", "resources")
	: process.resourcesPath;

await downloadAndAssemble(manifestUrl, resourcesDir)
	.then(async update => {
		if (update) {
			await import("./update");
			process.exit(0);
